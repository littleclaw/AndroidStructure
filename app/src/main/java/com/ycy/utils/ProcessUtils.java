...
public class ProcessUtils {
    ...
    public static String getForegroundProcessName() {
        ActivityManager manager = (ActivityManager) YcyApplication.getInstance().getSystemService(Context.ACTIVITY_SERVICE);
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            PackageManager packageManager = YcyApplication.getInstance().getPackageManager();
            Intent intent = new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS);
            List<ResolveInfo> list = packageManager.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
            System.out.println(list);
            if (list.size() > 0) {// 有"有权查看使用权限的应用"选项
                try {
                    ApplicationInfo info = packageManager.getApplicationInfo(YcyApplication.getInstance().getPackageName(), 0);
                    AppOpsManager aom = (AppOpsManager) YcyApplication.getInstance().getSystemService(Context.APP_OPS_SERVICE);
                    ...
                    UsageStatsManager usageStatsManager = (UsageStatsManager) YcyApplication.getInstance().getSystemService(Context.USAGE_STATS_SERVICE);
    ...
    public static Set<String> getAllBackgroundProcesses() {
        ActivityManager am = (ActivityManager) YcyApplication.getInstance().getSystemService(Context.ACTIVITY_SERVICE);
